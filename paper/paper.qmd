...
income %>%
  slice(1:20) %>%
  kable(
    caption = "Extracting the first twenty rows from the Income data",
...
income3 <- income3 %>% 
  rename(`Geographical Location` = Geographical_Location)
...
income4 %>%
  filter(
         ...
         `Geographical Location` != "Canada") %>%
  ggplot(aes(x=Year, y=`Highest-to-Lowest_Average_Income_Ratio`, colour = `Geographical Location`))+geom_point()+theme_minimal()+ylab("Average after-tax income range")
```
...
#| warning: false
#| message: false
income5 <- read_parquet(file = here::here("data/analysis_data/cleaned_income_data.parquet"),show_col_types = FALSE)
income5 <- income5 %>% 
  rename(`Geographical Location` = Geographical_Location)
income5 %>%
  filter(
         `Income decile` == "Total deciles",
         `Geographical Location` != "Canada") %>%
  ggplot(aes(x=Year, y=`Income_Range`, colour = `Geographical Location`))+geom_point()+theme_minimal()+ylab("Average after-tax income range")
```
